...
IplImage* ImageUtils::cropRectangle(IplImage *img, CvRect region)
{
...
void ImageUtils::drawRectangle(IplImage *img, CvRect face, CvScalar col)	{
	CvPoint p1, p2;
	p1.x = face.x;
	p1.y = face.y;
...
IplImage* ImageUtils::resizeImage(IplImage *origImg, int newWidth, int newHeight)
{
...
IplImage* ImageUtils::rotateImage(IplImage *src, IplImage* dst, float angle, float scale)
{
...
int ImageUtils::saveImage(char *filename, IplImage *image)
{
	int ret = -1;
...
void ImageUtils::saveFloatMat(const char *filename, const CvMat *src)
{
	IplImage srcIplImg;
...
void ImageUtils::printImageInfo(IplImage *image_tile)
{
	if (image_tile)
