...
int main(int nArgs, char* args[]) {
	...
	if (nArgs == 2 && !strcmp(args[1], "recal")) {
		...
		calDatabaseParams(base_front_params, base_side_params);
		cout << "Completed !!!" << "\n";
		system("pause");
	}
	else {
		...
		loadDatabaseParams(base_front_params, base_side_params);
		std::cout << "Completed !!!" << "\n";
		...
		calFrontFaceParams(image_front, front_prams, SHOW_PROCESS);
		...
		if (false && detectSideFace(image_side, side_faces)) {
		...
		calSideFaceParams(prepared_image_side, side_prams, SHOW_PROCESS);

		/* Find closest model*/
		int idx = findClosestModel(base_front_params, base_side_params, front_prams, side_prams);
